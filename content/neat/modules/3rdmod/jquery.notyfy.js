...
(function (factory) {
    ...
    (function ($) {
        ...
        function NotyfyObject(options) {
            ...
            $.extend(self, {
                ...
                _build: function () {
                    ...
                    if (self.options.titleIconClass) {
                        bar.find('.notyfy_icon').toggleClass(self.options.titleIconClass, true);
                    }

                    if (self.options.title) {
                        bar.find('.notyfy_title').text(self.options.title);
                    }




                    if (self.options.detailButtons) {
                        bar.find('.notyfy_detail_button_container').append(
                            $.map(self.options.detailButtons, function (button, i) {

                                return $('<div/>', {
                                    'class': button.addClass || 'notyfy_detail_button',
                                    'html': button.text,
                                    'click': function () {
        ...
        // 用于控制显示多少个提醒框
        var recentRecentShowCache = $.notyfyRecentShowCache = {
            data: [],
            maxCount: 5, //最多显示5个
            setNew: function (id) {
               
                var count = recentRecentShowCache.data.push(id);
                var currentMaxCount = recentRecentShowCache.maxCount;
                if (count > currentMaxCount) {
                    var delItems = recentRecentShowCache.data.splice(0, count - currentMaxCount);
                    $.each(delItems, function (_, item){
                        $.notyfy.close(item);
                    });
                }
        ...
        var renderer = $.notyfyRenderer = {
            ...
            show: function (notyfy) {
                ...
                $.notyfyRecentShowCache.setNew(notyfy.options.id);
            },
        ...
        $.notyfy = {
            ...
            defaults: {
                ...
                dismissQueue: true,
                template: '<div class="notyfy_message"><div class="layui-row notify_title_container"><div class="layui-col-xs2" style="text-align:left"><div class="notyfy_icon"></div></div><div class="layui-col-xs8  notyfy_title" style="text-align:center"></div><div class="layui-col-xs2" style="text-align:right"><i class="fas fa-times notyfy_close"></i></div></div><div class="layui-row notyfy_body "><div class="layui-col-xs12  text-left"><div class="layui-row notyfy_text"></div><div class="layui-row notyfy_detail_button_container"></div></div></div></div>',
